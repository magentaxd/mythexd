...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}kayıt <@Ramal/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
   ...
   const tagModedata = await regstats.findOne({ guildID: message.guild.id })
    ...
    if(!uye.roles.cache.has("855159733048311818") && !uye.roles.cache.has(ayar.vipRole) && !uye.roles.cache.has(ayar.boosterRolu)) return message.lineReply(embed.setDescription(`${uye.toString()} isimli üyenin kullanıcı adında tagımız (\`୪, ୪, #0000\`) olmadığı, <@&${ayar.boosterRolu}>, <@&${ayar.vipRole}> Rolü olmadığı için isim değiştirmekden başka kayıt işlemi yapamazsınız.`));
    }
...
Üyesinin önceki isimlerine \`.isimler <@Ramal/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
    `)
